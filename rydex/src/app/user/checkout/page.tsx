  ...
  const handleRequestBooking = async () => {
    try {
      ...
      if (!driverId || !vehicleId) {
        ...
        toast.error("Unable to request ride: missing driver or vehicle");
        setLoading(false);
        return;
      }
      ...
      if (
        ...
        toast.error("Unable to request ride: invalid pickup/drop coordinates");
        setLoading(false);
        return;
      }
      ...
      if (!Number.isFinite(numericFare) || numericFare < 0) {
        ...
        toast.error("Unable to request ride: invalid fare");
        setLoading(false);
        return;
      }
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (paymentMethod === "cash") {
        try {
          const { data } = await axios.get(`/api/booing/${booking._id}/confirm`);

          if (data?.success) {
            toast.success("Cash ride confirmed");
            setStatus("confirmed");
          ...
          >
            ...
            {status === "confirmed" && (
              <motion.div
                ...
                className="space-y-6"
              >
                <div className="mx-auto w-16 h-16 rounded-2xl bg-emerald-600 text-white flex items-center justify-center shadow-lg">
                  <CheckCircle size={32} className="text-white" />
                </div>
                ...
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-black text-zinc-900">Ride Confirmed</h3>

                  <p className="text-sm text-zinc-500">
                    Your ride is confirmed. The driver will contact you shortly.
                  </p>
                </div>
                ...
                <div className="flex gap-4">
                  <button
                    onClick={() => router.push("/")}
                    className="flex-1 py-3 rounded-2xl bg-zinc-900 text-white font-bold"
                  >
                    Go Home
                  </button>
