...
const HeroSection = ({onAuthRequired} : {onAuthRequired:()=>void}) => {
 const { userData } = useSelector((state: RootState) => state.user);
  ...
  return (
    ...
    <motion.div initial={{opacity:0,y:38}}
    ...
    initial={{opacity:0}}
    animate={{opacity:1}}
    transition={{delay:0.6}} className='mt-4 max-w-xl text-gray-300'>
    ...
    <motion.button onClick={()=>!userData ? onAuthRequired() :route.push("/user/book") } whileHover={{scale:1.05}} whileTap={{scale:0.95}} className='mt-12 px-10 py-4 bg-white text-black rounded-full font-semibold shadow-xl'>
        Book Now
    </motion.button>
      </div>
    </div>
